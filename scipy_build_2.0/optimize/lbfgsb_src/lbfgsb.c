/* lbfgsb.f -- translated by f2c (version 20190311).
	...
	*col, doublereal *v, doublereal *p, integer *info)
{
    ...
    extern /* Subroutine */ int dtrsl__(doublereal *, integer *, integer *,
...
/*     Solve the triangular system */
    dtrsl__(&wt[wt_offset], m, col, &p[*col + 1], &c__11, info);
    if (*info != 0) {
	return 0;
    }
/*       solve D^(1/2)p1=v1. */
...
/*                    [  0         J'           ] [ p2 ]   [ p2 ]. */
/*       solve J^Tp2=p2. */
    dtrsl__(&wt[wt_offset], m, col, &p[*col + 1], &c__1, info);
    if (*info != 0) {
	return 0;
    }
	...
	integer *head, integer *info)
{
    ...
    extern /* Subroutine */ int dpofa_(doublereal *, integer *, integer *, 
	    ...
	    *, integer *), dtrsl__(doublereal *, integer *, integer *,
	    doublereal *, integer *, integer *);
...
/*        then form L^-1(-L_a'+R_z') in the (1,2) block. */
	...
	dtrsl__(&wn[wn_offset], &m2, col, &wn[js * wn_dim1 + 1], &c__11, info);
/* L71: */
    }
...
/*     Compute wv:=K^(-1)wv. */
    m2 = *m << 1;
    col2 = *col << 1;
    dtrsl__(&wn[wn_offset], &m2, &col2, &wv[1], &c__11, info);
    if (*info != 0) {
	return 0;
    }
    i__1 = *col;
...
/* L25: */
    ...
    dtrsl__(&wn[wn_offset], &m2, &col2, &wv[1], &c__1, info);
    if (*info != 0) {
	return 0;
    }
